# Documentation Index

This index is the starting point for using, operating, extending, and releasing AX Performance Advisor.

## Start Here

- [Installation](../INSTALL.md): install from release ZIP, verify, configure Codex, update and uninstall.
- [Architecture](architecture.md): components, data flow, trust boundaries, and output model.
- [Operations Guide](operations-guide.md): how to collect evidence, generate dashboards, and run read-only analysis.
- [Testing Guide](testing-guide.md): local validation, CI checks, dashboard smoke tests, and release verification.
- [Skills Catalog](skills-catalog.md): primary and advanced skill grouping.
- [Security Model](security-model.md): least-privilege SQL, evidence handling, masking, and secrets.
- [Threat Model](threat-model.md): risks and mitigations for evidence, scripts, dashboards, and integrations.
- [Release Runbook](release-runbook.md): package, validate, and publish the plugin.
- [Distribution Guide](distribution-guide.md): ZIP, GitHub/Azure DevOps, and Codex plugin distribution.
- [Documentation Flow Check](documentation-flow-check.md): audited reader flow and remaining documentation follow-ups.

## Feature Areas

- [AI/KI Features](ai-ki-features.md)
- [Additional AI/KI Extensions](ai-ki-extensions.md)
- [Autonomous Intelligence](autonomous-intelligence.md)
- [Autonomous Ops](autonomous-ops.md)
- [Enterprise Observability](enterprise-observability.md)
- [Advanced USPs](advanced-usps.md)
- [Governance Extensions](governance-extensions.md)
- [Strategy Extensions](strategy-extensions.md)
- [Learning Extensions](learning-extensions.md)
- [Market Differentiators](market-differentiators.md)
- [Platform Extensions](platform-extensions.md)
- [Future AI USPs](future-ai-usps.md)
- [AX Batch Collision Analysis](batch-collision-analysis.md)
- [Pipeline Orchestrator](pipeline-orchestrator.md)
- [USP Catalog](usp-catalog.md)

## Operational References

- [Finding Schema](finding-schema.md)
- [Action Playbooks](action-playbooks.md)
- [Ownership Map](ownership-map.md)
- [Admin Execution Mode](admin-execution-mode.md)
- [Optional Agent, RBAC, RAG, and Release Packaging](optional-agent-rbac-rag-release.md)
- [Web Portal and LLM Connector](web-portal-and-llm.md)
- [Service Hardening](service-hardening.md)

## GitHub / Contribution

- Root `README.md`: project overview and quick start.
- Root `CONTRIBUTING.md`: development workflow and standards.
- Root `SECURITY.md`: reporting and data-handling policy.
- `.github/`: CI workflow, issue templates, PR template, and repository hygiene.
